// Styles the SEO + Admin Guide tabs of the TFS Website review doc and replaces
// [[IMG:file.png]] markers with inline images uploaded to Drive.
...
const TARGET_TABS = ['t.7ly5ca1rk0au', 't.1w3q099frymf']
...
// 2. Collect unique image files referenced by markers across target tabs
const markerRe = /^\[\[IMG:([\w.\-]+)\]\]$/
const neededFiles = new Set()
for (const tab of allTabs) {
  if (!TARGET_TABS.includes(tab.tabProperties.tabId)) continue
...
// 4. Build + apply requests per tab
const IMG_W = 435
const IMG_H = 435 * (900 / 1440) // 271.875

for (const tabId of TARGET_TABS) {
  ...
  console.log(tabId, 'requests:', requests.length)
