import { useEffect, useState } from 'react'
import { Link, useParams, useSearchParams } from 'react-router-dom'
import SEO from '../components/SEO'
import ArticleCard from '../components/ArticleCard'
import Pagination from '../components/Pagination'
import RacingLoader from '../components/RacingLoader'
import { api } from '../lib/api'
import { useArticles } from '../hooks/useArticles'
import type { TeamPageMember } from '../lib/types'
import { displayAuthorName } from '../lib/formatAuthor'

const PAGE_SIZE = 12

export default function TeamMemberPage() {
  const { memberId } = useParams<{ memberId: string }>()
  const [searchParams, setSearchParams] = useSearchParams()
  const page = Math.max(1, Number(searchParams.get('page') || '1') || 1)
  const { articles, meta, fetchArticles } = useArticles()
  const [member, setMember] = useState<TeamPageMember | null>(null)
  const [loading, setLoading] = useState(true)
  const [notFound, setNotFound] = useState(false)

  useEffect(() => {
    let cancelled = false
    ;(async () => {
      if (!memberId) {
        setNotFound(true)
        setLoading(false)
        return
      }
      setLoading(true)
      setNotFound(false)
      try {
        const data = await api<{ member: TeamPageMember }>(
          `/api/team-page/${encodeURIComponent(memberId)}`,
        )
        if (cancelled) return
        const next = data.member
        setMember(next)
        if (next.authorId) {
          await fetchArticles({
            status: 'published',
            authorId: next.authorId,
            page,
            limit: PAGE_SIZE,
          })
        } else {
          await fetchArticles({
            status: 'published',
            authorName: next.name,
            page,
            limit: PAGE_SIZE,
          })
        }
      } catch {
        if (!cancelled) {
          setMember(null)
          setNotFound(true)
        }
      } finally {
        if (!cancelled) setLoading(false)
      }
    })()
    return () => {
      cancelled = true
    }
  }, [memberId, fetchArticles, page])

  const setPage = (p: number) => {
    const next = new URLSearchParams(searchParams)
    if (p <= 1) next.delete('page')
    else next.set('page', String(p))
    setSearchParams(next)
    window.scrollTo({ top: 0, behavior: 'smooth' })
  }

  if (loading) {
    return (
      <div className="max-w-5xl mx-auto px-4 py-16">
        <RacingLoader message="Loading articles..." />
      </div>
    )
  }

  if (notFound || !member) {
    return (
      <div className="max-w-lg mx-auto px-4 py-16 text-center">
        <SEO title="Team member not found" />
        <h1 className="text-2xl font-bold text-text-primary dark:text-white mb-2">
          Team member not found
        </h1>
        <Link to="/about" className="text-primary hover:underline text-sm font-semibold">
          Back to Meet the Team
        </Link>
      </div>
    )
  }

  const name = displayAuthorName(member.name)

  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      <SEO title={name} description={member.bio || `Articles by ${name} on The Fastest Sector.`} />

      <p className="mb-6">
        <Link
          to="/about"
          className="text-sm font-semibold text-primary hover:underline"
        >
          ← Meet the Team
        </Link>
      </p>

      <div className="flex flex-col sm:flex-row gap-5 items-start mb-10 pb-8 border-b border-gray-200 dark:border-white/10">
        {member.avatar ? (
          <img
            src={member.avatar}
            alt={name}
            className="w-24 h-24 rounded-full object-cover ring-2 ring-primary/20"
          />
        ) : (
          <div className="w-24 h-24 rounded-full bg-primary/10 flex items-center justify-center text-primary text-3xl font-bold">
            {name.charAt(0)}
          </div>
        )}
        <div>
          <p className="text-[11px] font-bold uppercase tracking-widest text-primary mb-1">
            Team Member
          </p>
          <h1 className="text-3xl font-black text-text-primary dark:text-white mb-2">{name}</h1>
          {member.roleTitle ? (
            <p className="text-xs font-semibold uppercase tracking-wide text-primary mb-2">
              {member.roleTitle}
            </p>
          ) : null}
          {member.bio ? (
            <p className="text-text-secondary dark:text-white/65 max-w-2xl leading-relaxed">
              {member.bio}
            </p>
          ) : null}
        </div>
      </div>

      <h2 className="text-xs font-black uppercase tracking-wider text-text-secondary dark:text-white/50 mb-4">
        Articles ({meta.total})
      </h2>

      {articles.length === 0 ? (
        <p className="text-text-secondary dark:text-white/50">No published articles yet.</p>
      ) : (
        <>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {articles.map((article) => (
              <ArticleCard key={article.id} article={article} />
            ))}
          </div>
          <Pagination
            className="mt-8"
            page={page}
            total={meta.total}
            pageSize={PAGE_SIZE}
            onChange={setPage}
          />
        </>
      )}
    </div>
  )
}
