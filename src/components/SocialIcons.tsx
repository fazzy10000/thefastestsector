...
type SocialKey = 'instagram' | 'tiktok' | 'linkedin' | 'twitter' | 'discord'

const SOCIAL_ORDER: { key: SocialKey; label: string; path: string }[] = [
  {
    key: 'instagram',
    label: 'Instagram',
...
export default function SocialIcons({
  ...
  const items = SOCIAL_ORDER.filter(({ key }) => Boolean(links?.[key]?.trim()))
  ...
  return (
    <div className={className}>
      {items.map(({ key, label, path }) => (
        ...
        >
          <svg className={iconClassName} fill="currentColor" viewBox="0 0 24 24" aria-hidden>
            <path d={path} />
          </svg>
        </a>
      ))}
...
export const SOCIAL_ADMIN_FIELDS: { key: SocialKey | 'email'; label: string }[] = [
